...
# Remove files renamed with DELETE_ prefix
Get-ChildItem -Path $audioDir -Recurse -Filter "DELETE_*" | Remove-Item -Force -ErrorAction SilentlyContinue
...
foreach ($dir in $dirs) {
    $files = Get-ChildItem -Path $dir.FullName -File | Where-Object { $_.Extension -match '\.(mp3|opus|wav|flac|m4a|aac)$' -and $_.Name -notmatch '^DELETE_' }
    foreach ($f in $files) {
